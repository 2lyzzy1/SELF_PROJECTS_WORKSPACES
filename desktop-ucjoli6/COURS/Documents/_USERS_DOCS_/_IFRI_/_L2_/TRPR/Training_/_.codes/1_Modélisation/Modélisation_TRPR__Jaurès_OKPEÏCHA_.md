...
- *Coûts des actions* (aléatoires):
  ...
  - Le non respect de la condition ($nbrMissionnaires \ge nbrCannibales$) annule tout les coûts ($cout = 0$), et une réussite est de 1 (finaliser la mission);

- *Description des transitions d'états*:    
    - Le nombre de missionnaires ou de cannibales varie entre 0 et 3 sur les deux rives
    - La barque est soit à gauche, soit à droite
    - Un retour de la barque, sur une rive, implique toujours d'avoir au plus deux et au moins un conducteur <!-- nautonier -->
...
>> - avec $t_i = 1$, $i \in \{1, 2, ..., n\}$, une traversée unique, et $NT$, le nombre de traversées.
